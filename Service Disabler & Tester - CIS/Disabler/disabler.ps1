# List of registry paths for services to disable
$registryPaths = @(
...
# Iterate over each registry path and disable the service
foreach ($regPath in $registryPaths) {
    # Check if the registry path exists
    if (Test-Path $regPath) {
        # Disable the service by setting the 'Start' DWORD value to 4 (disabled)
        Set-ItemProperty -Path $regPath -Name "Start" -Value 4
        Write-Output "$regPath has been disabled."
    } else {
